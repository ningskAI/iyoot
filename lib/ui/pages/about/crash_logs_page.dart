...
class _CrashLogsPageState extends State<CrashLogsPage> {
  ...
  Future<void> _loadCrashLogs() async {
    ...
    try {
      final files = await readService(
        AppServices.crashLogService,
      ).getCrashLogFiles();
      if (mounted) {
        setState(() {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('加载崩溃日志失败: $e')));
      }
    }
  }

  Future<void> _showCrashLogDetail(File file) async {
    final content = await readService(
      AppServices.crashLogService,
    ).readCrashLogFile(file);
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              await Share.share(content, subject: path.basename(file.path));
            },
            child: const Text('分享'),
          ),
          TextButton(
  ...
  Future<void> _clearAllCrashLogs() async {
    ...
    final success = await readService(
      AppServices.crashLogService,
    ).clearAllCrashLogs();
    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('崩溃日志已清除')));
        _loadCrashLogs();
      } else {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('清除崩溃日志失败')));
      }
    }
  }
  ...
  Future<void> _deleteCrashLog(File file) async {
    ...
    final success = await readService(
      AppServices.crashLogService,
    ).deleteCrashLogFile(file);
    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('崩溃日志已删除')));
        _loadCrashLogs();
      } else {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('删除崩溃日志失败')));
      }
    }
  }
