...
class FileUtils {
  ...
  /// 获取数据库路径
  static Future<String> getAppDatabasesPath() async {
    if (kIsAndroid) {
      final path = await database.getDatabasesPath();
      return path;
    } else {
      final documentsPath = await FileUtils.getDocumentsPath();
    ...
    await for (final entity in source.list(recursive: false)) {
      if (entity is File) {
        ...
        final destSubDir = Directory(
          path.join(dest.path, path.basename(entity.path)),
        );
