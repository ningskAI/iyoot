...
class StartupPerformanceMonitor {
  ...
  void end(String stage) {
    ...
    if (startTime != null) {
      ...
      PerformanceMonitor.instance.record(
        stage,
        duration: duration,
        scope: 'startup',
        category: LogCategory.startup,
        warningThreshold: const Duration(milliseconds: 800),
        logSuccess: AppLog.enableStartupLogs,
      );
    }
  }

  /// 计时一个同步或异步阶段。
  Future<T> trackAsync<T>(String stage, FutureOr<T> Function() action) async {
    start(stage);
    try {
      return await action();
    } finally {
